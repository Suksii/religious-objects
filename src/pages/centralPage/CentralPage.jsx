...
import './CentralPage.scss';
import {Link} from "react-router-dom";
const CentralPage = ({data}) => {
    const scrollToTop = () => {
        window.scrollTo({
            top: 0,
            behavior: "instant"
    ...
    return (
        <>
            <h1 className="title">Vjerski objekti srednje regije</h1>
                    ...
                    data.map((object) => {
                        return (
                            <div key={object.id} className="object">
                                ...
                                <Link to={`/srednja-regija/${object.name.toLowerCase().replaceAll(" ","-")}`}
                                      className="button"
                                      onClick={scrollToTop}>
                                    Detaljnije
                                </Link>
