...
import './NorthPage.scss';
import {Link} from "react-router-dom";
const NorthPage = ({data}) => {
    const scrollToTop = () => {
        window.scrollTo({
            top: 0,
            behavior: "instant"
    ...
    return (
        <>
            <h1 className="title">Vjerski objekti sjeverne regije</h1>
                    ...
                    data.map((object) => {
                        return (
                            <div key={object.id} className="object">
                                ...
                                <Link to={`/sjeverna-regija/${object.name.toLowerCase().replaceAll(" ","-")}`}
                                      className="button"
                                      onClick={scrollToTop}>
                                    Detaljnije
                                </Link>
