...
const ReligiousObject = ({ data, regionLink }) => {
  ...
  const ReligiousObject = data.find(
    (object) => object.name.toLowerCase().replaceAll(" ", "-") === objectId
  );
  ...
  return (
    <div style={{ padding: "20px 0" }}>
      ...
      <div className="container">
        <div className="image">
          {/* <ImageSlider
            images={ReligiousObject.images}
            currentIndex={currentIndex}
            setCurrentIndex={setCurrentIndex}
            imgUrl={ReligiousObject.images[currentIndex].image}
          /> */}
          <img
            src={ReligiousObject.images[0].image}
            alt={`${ReligiousObject.name}-${ReligiousObject.images[0].id}`}
          />
        </div>
        <div className="paragraphs">
          <div className="description">
            {ReligiousObject.descriptions?.map((description) => {
              return <p key={description?.id}>{description?.description}</p>;
            })}
          </div>
          <div className="sources">
            {ReligiousObject.sources?.map((source) => {
              return <p key={source?.id}>{source?.source}</p>;
            })}
          </div>
        </div>
      </div>
